...
```
CasE ──inhibits──▶ Csy4 ──inhibits──▶ PgU ──inhibits──▶ mNeonGreen (green)
     (Csy4_rec_CasE)    (PgU_rec_Csy4)    (PgU_rec_mNeonGreen)
```

### Step-by-step logic trace:

1. **CasE is expressed freely** (150 ng, highest dose) — no ERN is targeting it
2. **CasE cuts Csy4 mRNA** — the plasmid `Csy4_rec_CasE` encodes Csy4 but its mRNA has a CasE recognition sequence. CasE finds it and destroys it. Csy4 protein production is suppressed.
3. **Csy4 is gone, so PgU survives** — the plasmid `PgU_rec_Csy4` encodes PgU with a Csy4 recognition sequence. But Csy4 has been killed by CasE, so nothing is cutting PgU's mRNA. PgU is produced normally.
4. **PgU cuts mNeonGreen mRNA** — the plasmid `PgU_rec_mNeonGreen` encodes mNeonGreen with a PgU recognition sequence. PgU is active and destroys the mNeonGreen mRNA.
...
- **mNeonGreen (green)**: Regulated by PgU. PgU is active → green is **OFF**.
...
### Group X1: CasE + eBFP2 (co-transfected)

| Plasmid | Amount | Role |
|---------|--------|------|
| CasE | 150 ng | The dominant ERN enzyme. Input signal for the cascade. |
| eBFP2 | 50 ng | Blue fluorescent reporter. Constitutive control — confirms transfection worked. |

These two plasmids are mixed together in the same tube and delivered to the cell as a single lipid-DNA complex. CasE is given the highest dose (150 ng) to ensure strong suppression of its target (Csy4).

### Group X2: Csy4_rec_CasE + mMaroon1 (co-transfected)

| Plasmid | Amount | Role |
|---------|--------|------|
| Csy4_rec_CasE | 100 ng | Encodes Csy4 enzyme, but mRNA has CasE recognition site. CasE will cut this mRNA, preventing Csy4 production. |
| mMaroon1 | 50 ng | Maroon/red fluorescent reporter. Constitutive control — confirms transfection worked. |

Csy4 would normally be an active enzyme, but because its mRNA is tagged with a CasE recognition sequence, CasE from Group X1 destroys it. This is the first inhibition step.

### Group X3: PgU_rec_Csy4 (single transfection)

| Plasmid | Amount | Role |
|---------|--------|------|
| PgU_rec_Csy4 | 100 ng | Encodes PgU enzyme, but mRNA has Csy4 recognition site. If Csy4 were present, it would cut this mRNA. But Csy4 has been killed by CasE, so PgU is produced normally. |

This is the second layer of the cascade. PgU survives because its inhibitor (Csy4) has been suppressed by CasE. PgU is now free to act on its target.

### Group Bias: PgU_rec_mNeonGreen (single transfection)

| Plasmid | Amount | Role |
|---------|--------|------|
| PgU_rec_mNeonGreen | 200 ng | Encodes mNeonGreen (green fluorescent protein), but mRNA has PgU recognition site. PgU is active and cuts this mRNA, so mNeonGreen is NOT produced. |

This is the output of the cascade. mNeonGreen is given the highest dose (200 ng) among the regulated parts, but it doesn't matter — PgU is actively destroying its mRNA. The high dose means that if PgU *weren't* present, we'd see very strong green fluorescence. Its absence is the signal that the cascade is working.
...
| PgU_rec_mNeonGreen | 200 | 0.308 | 30.8% |
...
| A3 | Csy4_rec_CasE DNA (50 ng/uL) | Source tube |
| A4 | mMaroon1 DNA (50 ng/uL) | Source tube |
| A5 | PgU_rec_Csy4 DNA (50 ng/uL) | Source tube |
| A6 | PgU_rec_mNeonGreen DNA (50 ng/uL) | Source tube |
| B1 | Empty → X1 DNA mix (CasE + eBFP2) | DNA destination |
| B2 | Empty → X2 DNA mix (Csy4_rec_CasE + mMaroon1) | DNA destination |
| B3 | Empty → X3 DNA mix (PgU_rec_Csy4) | DNA destination |
| B4 | Empty → Bias DNA mix (PgU_rec_mNeonGreen) | DNA destination |
...
|---------|-----------|---------------|------------|-------------|
| CasE | 150 | 50 ng/uL | 3.6 uL | (150/50) × 1.2 |
...
| PgU_rec_mNeonGreen | 200 | 50 ng/uL | 4.8 uL | (200/50) × 1.2 |
...
A5 (PgU_rec_Csy4, 2.4 uL)  ──▶ B3 (Group X3 mix)

A6 (PgU_rec_mNeonGreen, 4.8 uL) ──▶ B4 (Group Bias mix)
```

Uses the p20 pipette (left mount) for all transfers since volumes are under 20 uL. Each source tube is mixed 3× before aspiration. Co-transfection groups are mixed 3× after the last plasmid is added.
...
2. Robot pipettes 46.8 uL Opti-MEM from D6 → D2, mixes 3×
...
2. Robot pipettes 46.8 uL Opti-MEM from D6 → D1, mixes 3×
...
Each tube is mixed 3× after combining. The lipids self-assemble around the DNA into nanoparticles.
...
| Time | What happens |
|------|-------------|
| 0 hours | Plate goes into 37°C / 5% CO2 incubator |
...
| 12-24 hours | ERN enzymes accumulate to functional levels. CasE starts cutting Csy4 mRNA. PgU starts cutting mNeonGreen mRNA. Circuit logic takes full effect. |
...
| Green | mNeonGreen | **OFF** (dark) | PgU is active and cutting its mRNA. This is the circuit output. |

### ERN Activity Summary

| ERN | Expressed from | Targeted by | Status | Effect |
|-----|---------------|-------------|--------|--------|
| CasE | CasE (free) | Nothing | **ACTIVE** | Cuts Csy4 mRNA |
| Csy4 | Csy4_rec_CasE | CasE | **SUPPRESSED** | Cannot cut PgU mRNA |
| PgU | PgU_rec_Csy4 | Csy4 (suppressed) | **ACTIVE** | Cuts mNeonGreen mRNA |

## Comparison with Default Circuit

| | Default (MyCircuit) | Ours (ThreeLayer) |
|---|---|---|
| Layers | 2 (Csy4 → CasE) | 3 (CasE → Csy4 → PgU) |
| ERNs used | 2 (Csy4, CasE) | 3 (CasE, Csy4, PgU) |
...
1. **Remove CasE**: Without CasE, Csy4 is free → Csy4 kills PgU → PgU can't kill mNeonGreen → green ON
2. **Remove Csy4_rec_CasE**: CasE has nothing to inhibit → PgU_rec_Csy4 has no Csy4 to fear → PgU is active → green OFF (same as full circuit, but simpler)
3. **Remove PgU_rec_Csy4**: No PgU → nothing cuts mNeonGreen → green ON
